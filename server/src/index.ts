...
// --- Category CRUD (Public READ, Admin MUTATION) ---
...
// --- Food CRUD (Public READ, Admin MUTATION) ---
  ...
  try {
    ...
    const [foods, total] = await Promise.all([
      prisma.food.findMany({
        where, skip, take: limitNum, include: { category: true }, orderBy: { createdAt: 'desc' },
      }),
...
app.post('/api/foods', authenticateToken, isAdmin, async (req, res) => {
  try {
...
app.put('/api/foods/:id', authenticateToken, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, price, imageUrl, categoryId } = req.body;
...
app.delete('/api/foods/:id', authenticateToken, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
